...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function totalInvestmentsPlusMilestoneAmount()
    {
        return Investment::where(['user'=>$this->email,'paid'=>0])->where('status', '!=' ,'pending')->sum('amount_invested') + MilestoneInvestment::where(['user_id'=> auth()->user()['id'],'paid'=>0])->where('status', '!=' ,'pending')->sum('amount_invested');
    }

    public function totalInvestmentsPlusMilestoneAmountAndPaid()
    {
        return Investment::where(['user'=>$this->email,'status'=>'active'])->sum('amount_invested') + MilestoneInvestment::where(['user_id'=> auth()->user()['id'],'status'=>'active'])->sum('amount_invested');
    }

    public function totalUnitsPlusMilestoneAmount()
    {
        return Investment::where(['user'=>$this->email,'status'=>'active'])->sum('units') + MilestoneInvestment::where(['user_id'=> auth()->user()['id'],'status'=>'active'])->sum('units');
    }

    public function hasCompletedProfile()
    {
        return $this->address != '' && $this->state != '' && $this->country != '' && $this->city != '' && $this->dob != '';
    }
