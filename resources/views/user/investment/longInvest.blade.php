...
{{--                        </div>--}}
                        <br>
						<div class="form-group">
							<button id="investButton" disabled onclick="return confirm('You won\'t be able to cancel this once started. Are you sure you want to proceed?');" type="submit" class="btn btn-success btn-block btn-lg">Make Investment</button>
						</div>
					</form>
				</div>
			</div>
...
@section('foot')
    <script>
        ...
        let walletBalance = {{ \App\Wallet::where('user', auth()->user()['email'])->first()['total_amount'] }};
        let toPay = 0;
